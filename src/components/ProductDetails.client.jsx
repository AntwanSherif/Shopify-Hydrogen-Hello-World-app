        ...
        <div className='grid items-start gap-6 lg:gap-20 md:grid-cols-2 lg:grid-cols-3'>
          ...
          <div className='sticky md:mx-auto max-w-xl md:max-w-[24rem] grid gap-8 p-0 md:p-6 md:px-0 top-[6rem] lg:top-[8rem] xl:top-[10rem]'>
            <div className='grid gap-2'>
              <h1 className='text-4xl font-bold leading-10 whitespace-normal'>{product.title}</h1>
          ...
          {options.map(({ name, values }) => {
            ...
            return (
              <div key={name} className='flex flex-wrap items-baseline justify-start gap-6'>
                <legend className='whitespace-pre-wrap max-w-prose font-bold text-lead min-w-[4rem]'>{name}</legend>
